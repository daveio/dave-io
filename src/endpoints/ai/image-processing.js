...
export class ImageProcessor extends AiAltBase {
     ...
     */
    async processImageFromUrl(c, imageUrl) {
        try {
            ...
            if (!hasValidExtension) {
                return this.createErrorResponse(c, "Invalid image URL - URL must point to a valid image file", "INVALID_IMAGE_URL");
            }
            // Fetch the image
            const response = await fetch(imageUrl);
            if (!response.ok) {
                return this.createErrorResponse(c, `Failed to fetch image: ${response.status} ${response.statusText}`, "FETCH_ERROR");
            }
     ...
     */
    processBase64Image(c, imageDataUrl) {
        // Check if it's a valid data URL
        if (!imageDataUrl.startsWith("data:image/")) {
            return this.createErrorResponse(c, "Invalid image data. Must be a base64 data URL starting with 'data:image/'", "INVALID_IMAGE_DATA");
        }
