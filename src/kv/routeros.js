 ...
 */
export async function getCacheData(env) {
    // Default empty cache
    const defaultCache = {
        ipv4Ranges: [],
        ipv6Ranges: [],
    ...
    try {
        ...
        const [ipv4Ranges, ipv6Ranges] = await Promise.all([
            env.DATA.get(KV_CACHE_IPV4, { type: "json" }),
            env.DATA.get(KV_CACHE_IPV6, { type: "json" })
        ]);
        return {
 ...
 */
export function shouldRefreshCache(cacheData) {
 ...
 */
export function isCacheStale(cacheData) {
    if (!cacheData.lastUpdated) {
        return true;
    }
    const lastUpdated = new Date(cacheData.lastUpdated);
 ...
 */
export async function getSharedMetadata(env) {
    try {
        // Get all metrics keys individually
        const [lastRefresh, refreshCount, cacheResets, cacheHits, cacheMisses, lastAccessed, lastReset, resetCount] = await Promise.all([
 ...
 */
export async function updateSharedMetadata(env, metrics, expirationTtl) {
 ...
 */
async function fetchBGPViewData() {
 ...
 */
function processIPRanges(ripeData, bgpViewData, version) {
 ...
 * Merge IP ranges
 */
function mergeIPRanges(ranges, version) {
    if (ranges.length === 0) {
        return [];
    }
    try {
 ...
 */
export function generateScript(cacheData) {
...
export async function refreshCache(env) {
    ...
    try {
        ...
        const refreshCount = (sharedMetadata.refreshCount || 0) + 1;
        await Promise.all([
 ...
 */
export async function getScript(env) {
    try {
        // Track the request
        env.ANALYTICS.writeDataPoint({
