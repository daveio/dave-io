 ...
 */
export async function incrementStatusCodeCount(env: { DATA: KVNamespace }, statusCode: number): Promise<void> {
  try {
 ...
 */
async function incrementStatusGroupCount(env: { DATA: KVNamespace }, statusCode: number): Promise<void> {
  try {
    // Determine group
 ...
 */
export async function getStatusCodeCount(env: { DATA: KVNamespace }, statusCode: number): Promise<number> {
  try {
    const key = `${STATUS_PREFIX}${statusCode}`
 ...
 */
export async function getStatusGroupCount(env: { DATA: KVNamespace }, group: string): Promise<number> {
  try {
    const key = `${GROUP_PREFIX}${group}`
    const count = await env.DATA.get(key)
    return count ? Number.parseInt(count, 10) : 0
