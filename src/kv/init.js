...
import { KV_CACHE_IPV4, KV_CACHE_IPV6, KV_CACHE_SCRIPT, KV_METRICS_CACHE_HITS, KV_METRICS_CACHE_MISSES, KV_METRICS_CACHE_RESETS, KV_METRICS_LAST_ACCESSED, KV_PUTIO_METADATA_LAST_ATTEMPT, KV_PUTIO_METADATA_LAST_ERROR, KV_PUTIO_METADATA_LAST_UPDATED, KV_PUTIO_METADATA_UPDATE_IN_PROGRESS } from "./routeros";
/**
 * Initialize KV stores with default values if they don't exist
 */
export async function initializeKV(env) {
 ...
 */
async function initializeRouterOSKV(env) {
    ...
    const ipv4Ranges = await env.DATA.get(KV_CACHE_IPV4, { type: "json" });
    if (!ipv4Ranges) {
        await env.DATA.put(KV_CACHE_IPV4, JSON.stringify([]));
    }
    // Initialize IPV6 ranges if not exist
    const ipv6Ranges = await env.DATA.get(KV_CACHE_IPV6, { type: "json" });
    if (!ipv6Ranges) {
 ...
 */
async function initializeRedirectsKV(env) {
