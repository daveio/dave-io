...
async function exportKVKeysToYAML() {
  try {
    ...
    try {
      ...
      console.log(`🔑 Found ${keyObjects.length} keys in the KV namespace`);
      ...
      for (let i = 0; i < keyObjects.length; i += batchSize) {
        ...
        await Promise.all(batch.map(async (keyObj) => {
          const keyName = keyObj.name;
          try {
            // Fetch the value for this key
            const valueResult = await $`bun run wrangler kv key get "${keyName}" --binding DATA --remote | cat`.quiet();
            
            if (valueResult.exitCode !== 0) {
              console.warn(`⚠️ Failed to fetch value for key "${keyName}". Skipping.`);
              return;
            }
            ...
            console.warn(`⚠️ Error fetching value for key "${keyName}":`, error);
          } finally {
            completedCount++;
            if (completedCount % 10 === 0 || completedCount === totalCount) {
              console.log(`⏳ Progress: ${completedCount}/${totalCount} keys processed (${Math.round(completedCount/totalCount*100)}%)`);
            }
          }
        }));
      }
      
      console.log(`✅ All ${completedCount} values fetched successfully`);
      ...
      console.log(`✅ KV keys and values successfully exported to: ${outputPath}`);
    } catch (parseError) {
      console.error("❌ Error parsing JSON:", parseError);
      console.error("JSON text extracted:", jsonText);
      process.exit(1);
    }
  } catch (error) {
